...
# AUTHOR
lim = 0.7
#authors = ['alpha', 'bravo', 'charlie', 'delta', 'echo', 'foxtrot', 'golf', 'hotel', 'india', 'juliet', 'kilo', 'lima', 'mike', 'november', 'oscar', 'papa', 'romeo', 'quebec', 'sierra', 'tango', 'uniform', 'victor', 'whiskey', 'xray', 'yankee', 'zulu']
...
goaldirpath = os.getcwd() + '/author_package'
...
for file in os.listdir(goaldirpath):
    ...
    if os.path.isdir(filepath):
        ...
        # AUTHOR
        classBadDocumented_rank_out_path = packagedirpath + '_classBadDocumented_rank_BI.json'
        classBadDocumented_rank_temp = {}
        classBadDocumented_rank_out = open(classBadDocumented_rank_out_path, 'w')
        # AUTHOR
        ...
        for file2 in os.listdir(packagedirpath):
            ...
            if os.path.isfile(file2path) and not file2path.endswith("_BI.json"):
                ...
                with open(classfilepath) as data_file:
                    ...
                    classfile_out_temp.update({'author': author})

                    if classfile_out_temp['throws']['%c'] < lim:
                        java_name = data_file_name[:-5] + ".java"
                        classBadDocumented_rank_temp.update({java_name: {'author': author, '%c': classfile_out_temp['throws']['%c']}})
                    # AUTHOR
        ...
        # AUTHOR
        print(classBadDocumented_rank_temp)
        classBadDocumented_rank_out.write(json.dumps(classBadDocumented_rank_temp, indent=2, sort_keys=True))
        classBadDocumented_rank_out.close()
        # AUTHOR
